...
@section('content')
    <div class="container mx-auto p-6 dark:bg-gray-800 dark:text-white">
        <div
            class="flex flex-col md:flex-row justify-between bg-white dark:bg-gray-700 p-4 m-4 rounded-md shadow-md border border-gray-300 dark:border-gray-600">
            <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-200">{{ __('messages.edit_form') }} {{$form->name}}</h1>
            <a onclick="window.history.back();"
               class="btn add-option bg-gray-600 text-white px-2 py-1 rounded hover:bg-gray-400 dark:bg-gray-500 dark:hover:bg-gray-400 mt-2 md:mt-0">{{ __('messages.back') }}</a>
        </div>

        <form action="{{ route('form.update',$form->id) }}" method="POST"
              class="mb-6 bg-white dark:bg-gray-700 p-4 rounded-md shadow-md border border-gray-300 dark:border-gray-600 m-4"
            ...
            <div class="mb-4">
                <label for="description"
                       class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('messages.description') }}</label>
                <textarea
                    class="mt-1 block w-full px-4 py-2 border-gray-300 bg-gray-100 dark:bg-gray-600 dark:border-gray-500 text-gray-800 dark:text-gray-200 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200"
        ...
        <form method="POST" action="{{ route('form.update', $form->id) }}"
            ...
            <div>
                    ...
                    @foreach ($form_fields as $field)
                        <div class="field-item mb-4 border rounded p-4 bg-gray-200 dark:bg-gray-800">
                            ...
                            <div class="flex flex-col mb-3">
                                <label
                                    class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('messages.field_name') }}</label>
                                ...
                                <label
                                    class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('messages.field_key') }}</label>
                                <input type="text" required name="fields[{{ $loop->index }}][key]"
                                       value="{{ $field->key }}"
                                       class="block w-full rounded-md border-gray-300 bg-gray-100 dark:bg-gray-600 dark:border-gray-500 text-gray-800 dark:text-gray-200 shadow-sm focus:border-teal-500 focus:ring-teal-500 sm:text-sm mb-3 p-2">

                                <label
                                    class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('messages.tooltip') }}</label>
                                <input type="text" name="fields[{{ $loop->index }}][tooltip]"
                                       value="{{ $field->tooltip }}"
                                       class="block w-full rounded-md border-gray-300 bg-gray-100 dark:bg-gray-600 dark:border-gray-500 text-gray-800 dark:text-gray-200 shadow-sm focus:border-teal-500 focus:ring-teal-500 sm:text-sm mb-3 p-2">
                                <label
                                    class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('messages.sequence') }}</label>
                            ...
                            <label
                                class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-1">{{ __('messages.field_type') }}</label>

                            <select name="fields[{{ $loop->index }}][type]" data-loop-index="{{ $loop->index }}"
                            ...
                            @if ($field->options)
                                @if($field->fieldTypes->name == 'Form' or $field->fieldTypes->name == 'Form Select')
                                    @php
                                        $form_id = json_decode($field->options);
                                        if ($form_id){
                                    ...
                                    <div class="options-container">
                                        <select id="option_select{{ $loop->index }}"
                                ...
                                @else
                                    <div class="options-container">
                                        <label
                                            class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('messages.options') }}
                ...
                <div class="flex flex-col md:flex-row mt-4">
                    ...
                    <button type="submit"
                            class="bg-teal-500 text-white px-4 py-2 rounded hover:bg-teal-600 dark:bg-teal-600 dark:hover:bg-teal-500 m-2">{{ __('messages.save') }}
                    </button>
                </div>
            </div>
        </form>
            ...
            document.getElementById('add-field').addEventListener('click', function () {
                ...
                <div class="field-item mb-4 border rounded p-4 bg-gray-200 dark:bg-gray-800">
                    ...
                    <div class="flex flex-col mb-3">
                        ...
                        <input type="text" name="fields[${fieldIndex}][label]" class="block w-full rounded-md border-gray-300 bg-gray-100 dark:bg-gray-600 dark:border-gray-500 text-gray-800 dark:text-gray-200 shadow-sm focus:border-teal-500 focus:ring-teal-500 sm:text-sm mb-3 p-2" placeholder="{{ __('messages.field_name') }}" required>

                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('messages.field_key') }}</label>
                        <input type="text" name="fields[${fieldIndex}][key]" class="block w-full rounded-md border-gray-300 bg-gray-100 dark:bg-gray-600 dark:border-gray-500 text-gray-800 dark:text-gray-200 shadow-sm focus:border-teal-500 focus:ring-teal-500 sm:text-sm mb-3 p-2" placeholder="{{ __('messages.field_key') }}" required>

                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('messages.tooltip') }}</label>
                        <input type="text" name="fields[${fieldIndex}][tooltip]" class="block w-full rounded-md border-gray-300 bg-gray-100 dark:bg-gray-600 dark:border-gray-500 text-gray-800 dark:text-gray-200 shadow-sm focus:border-teal-500 focus:ring-teal-500 sm:text-sm mb-3 p-2" placeholder="{{ __('messages.tooltip') }}">

                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('messages.sequence') }}</label>
                        <input type="number" name="fields[${fieldIndex}][sequence]" class="block rounded-md border-gray-300 bg-gray-100 dark:bg-gray-600 dark:border-gray-500 text-gray-800 dark:text-gray-200 shadow-sm focus:border-teal-500 focus:ring-teal-500 sm:text-sm mb-3 p-2" placeholder="{{ __('messages.sequence') }}" required>
                    </div>

                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('messages.field_type') }}</label>
                    <select name="fields[${fieldIndex}][type]" data-loop-index=${fieldIndex} class="type-field-select block w-full rounded-md border-gray-300 dark:border-gray-500 bg-teal-500 text-gray-100 shadow-sm focus:border-teal-500 focus:ring-teal-500 sm:text-sm mb-3 p-2 dark:bg-gray-600 dark:text-gray-200">
                        @foreach ($fieldTypes as $fieldType)
